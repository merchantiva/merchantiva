...
> **Note:** this internal pipeline is a close cousin of, but not identical to, the customer-facing 7-stage lifecycle in `docs/lifecycle.md` (Discover → Validate → Launch → Acquire → Scale → Expand → Exit). The pipeline above describes how the *team* produces a venture; `lifecycle.md` describes how the *customer* experiences their journey. A customer can enter the customer lifecycle at any stage without Merchantiva running the full production pipeline for them — e.g. a customer might enter at "Acquire Customers" because they already have a store, in which case Merchantiva only runs the Growth/Optimization portion of the internal pipeline, not the whole thing. Keep both maps — don't merge them — but be aware they're different views of overlapping territory, and don't let site or marketing copy quote this internal pipeline as if it were the customer-facing lifecycle. Building the company around this internal pipeline instead of around customer needs is the mistake this distinction exists to prevent.
